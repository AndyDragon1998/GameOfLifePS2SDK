...
{	
	...
	strcpy(introTrack, bgm->fileName);
	strcat(introTrack, "_Intro.wav");
	strcpy(loopTrack, bgm->fileName);
	strcat(loopTrack, "_Loop.wav");
	if (bgm->ret != 0)
	{
	...
	if(!(bgm->wav = fopen(introTrack, "rb")))
	{
		LoopingPart = 1;
	}
